    ...
    public CropPane()  {
     /*   primaryStage.setTitle(APPLICATION_NAME);
       ...
       // final Scene scene =  new Scene(borderPane, 800, 800);
        ...
        mainImage=convertFileToImage(Launcher.imageFiles.get(0));
        ...
        MenuItem nextImage=new Menu("Next Image");
        nextImage.setOnAction(e->{
            if(imageIndex>=Launcher.imageFiles.size()-1){
                imageIndex=0;
            }else {
                imageIndex++;
                mainImageView.setImage(convertFileToImage(Launcher.imageFiles.get(imageIndex)));
            }
        });
        MenuItem save=new Menu("Save");
        menu1.getItems().addAll(open,clear,nextImage);
        ...
        open.setOnAction(event -> {
            ...
            if (selectedFile != null) {
                ...
                mainImageView.setImage(mainImage);
                changeStageSizeImageDimensions(Launcher.primaryStage,mainImage);
            }
        });
...
*/
    ...
    private Image convertFileToImage(File imageFile) {
        ...
        try (FileInputStream fileInputStream = new FileInputStream(imageFile)){
            image = new Image(fileInputStream,400,400,false,false);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return image;
